...
shaka.dash.MpdUtils.fillUriTemplate = function(
  ...
  var re = /\$(RepresentationID|Number|Bandwidth|Time)?(?:%0([0-9]+)([dxXoO]))?\$/g;  // gjslint: disable=110
  var uri = uriTemplate.replace(re, function(match, name, widthString, radixString) { // gjslint: disable=110
    if (match == '$$') {
      return '$';
    }
    ...
    if (name == 'Number' && radixString) {
      var radix = radixTable[radixString];
      var valueString = value.toString(radix);
      if (radixString === radixString.toUpperCase()) {
