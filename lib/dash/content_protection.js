 ...
 *   Contains a variable number of records that contain license acquisition
 *   information.
 */
 ...
 */
shaka.dash.ContentProtection.parseRecords = function(recordData, recordCount) {
  var head = 0;
  var records = [];
 ...
 */
shaka.dash.ContentProtection.parsePro = function(bytes) {
  var length = bytes[0] | bytes[1];
  var recordCount = bytes[2];

  var recordData = bytes.subarray(3);
  var records = shaka.dash.ContentProtection.parseRecords(recordData,
                                                          recordCount);
 ...
 * @return {string}
 */
shaka.dash.ContentProtection.getLaurl = function(xml) {
 ...
 * @return {string}
 */
shaka.dash.ContentProtection.getPRLicenseServerURL = function(element) {
  ...
  var parsedPro = shaka.dash.ContentProtection.parsePro(new Uint16Array(
                                                           bytes.buffer));
  ...
  for (var i = 0; i < records.length; i++) {
    ...
    if (record.type ===
        shaka.dash.ContentProtection.PLAYREADY_RECORD_TYPES.RIGHTS_MANAGEMENT) {
      var xml = parser.parseFromString(record.value,
                                       'application/xml').documentElement;

      return shaka.dash.ContentProtection.getLaurl(xml);
    }
  }
        ...
        if (keySystem) {
          ...
          if (keySystem === 'com.widevine.alpha') {
            drmInfo.licenseServerUri =
                shaka.dash.ContentProtection.getWidevineLicenseUrl(element);
          }

          // extract PlayReady license URL from PlayReady header Object
          if (keySystem === 'com.microsoft.playready') {
            drmInfo.licenseServerUri =
                shaka.dash.ContentProtection.getPRLicenseServerURL(element);
          }
